...
def get_losses(loss_name, batch, sc_angs_true, sc_angs_pred):
    if loss_name == "mse":
        return mse_over_angles(sc_angs_true, sc_angs_pred)
    if loss_name == "openmm":
        proteins = [SCNProtein(angles=a.copy(), seq=str(s), id=i) for (a, s, i) in zip(
            batch.angs, batch.str_seqs, batch.pids)]
        for p in proteins:
            # Fill in protein obj with predicted angles
            p.angles[:, 6:] = sc_angs_pred

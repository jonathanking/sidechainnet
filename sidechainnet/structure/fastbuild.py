from .build_info import SC_BUILD_INFO, BB_BUILD_INFO, NUM_COORDS_PER_RES, SC_ANGLES_START_POS, NUM_ANGLES
import numpy as np
import torch
import time
...
# For every sidechain atom need to know what atom it is building off of (<NUM_COORDS_PER_RES=14), the bond length, 
# the theta bond angle, and the chi torsion angle.  For atoms where the torsion angles are inputs, need to specify 
# angle source index; index should should start from zero as it is assumed that only relevant angles will be passed in.
...
def make_backbone_save(M):
    '''Make backbone matrices from individual transformation matrices using
    log(N) parallel matrix multiplies.  Does not modify in place.
    Not very memory efficient in the interest of simplicity.  Returns
    transformation matrices at each interation (last is final result).
    '''
...
def make_sidechain_coords(backbone, seq_aa_index, ang, build_info):
    '''given relevant backbone atom matrices, map from sequence to residue type,
     relevant variable angles, and  build info for chain of atoms off of residue, 
     create coordinates'''
    dtype = ang.dtype
    device = ang.device
    L = len(seq_aa_index)
    ...
    bond_lengths = build_info['bond_lengths'][seq_aa_index].to(device).type(dtype) # L x 10
